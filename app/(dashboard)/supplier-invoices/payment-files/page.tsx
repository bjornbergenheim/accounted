...
export default function PaymentFilesPage() {
  ...
  // Payment-initiation copy lives in its own namespace: the same sentences
  // appear on the skattekonto and salary screens, and three copies of a
  // sentence about money that may or may not have moved is three chances to
  // say something different.
  const tp = useTranslations('bank_payments')
  ...
  // Payment initiation (PSD2). Renders nothing at all unless the server says
  // the whole chain is in place, so the download flow below is untouched on an
  ...
  function canSend(batch: Pick<SupplierPaymentBatch, 'id' | 'status'>): boolean {
    return canWrite && payments.canSend(batch.id, batch.status === 'created')
  }

  /**
   * The redirect to BankID is the success path, so nothing is reported as done
   * here: the money has not moved until the bank says so, and the callback is
   * what records that. Every refusal is toasted by the hook with its own
   * sentence.
   */
  async function handleSendToBank() {
    if (!confirmSendBatch) return
    const started = await payments.send(confirmSendBatch.id)
    if (!started) setConfirmSendBatch(null)
  }
  ...
  return (
    <div className="space-y-8">
              ...
              {batches.map((batch) => (
                ...
                >
                  ...
                  <td className={cn(TD_CLASS, 'whitespace-nowrap')}>
                    {batch.status === 'cancelled' ? (
                    ...
                    ) : payments.orderFor(batch.id) && payments.orderFor(batch.id)!.status !== 'accepted' ? (
                      ...
                      <Badge variant="outline" className="font-normal">
                        {payments.orderLabel(payments.orderFor(batch.id)!.status)}
                      </Badge>
                    ) : (
          ...
          {detail ? (
            <>
              ...
              <SlideOverBody className="space-y-4">
                ...
                {payments.orderFor(detail.id) && (
                  <p className="attn">
                    {tp('order_line', {
                      status: payments.orderLabel(payments.orderFor(detail.id)!.status),
                    })}
                    {payments.orderFor(detail.id)!.error_message ?? ''}
                  </p>
                )}
              </SlideOverBody>
              <SlideOverFooter>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open && !payments.sending) setConfirmSendBatch(null)
        }}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{tp('confirm_title')}</DialogTitle>
            <DialogDescription>
              {tp('confirm_body', {
          ...
          <p className="text-xs text-muted-foreground">{tp('confirm_note')}</p>
          <DialogFooter>
            <Button
              variant="ghost"
            ...
            <Button onClick={handleSendToBank} disabled={payments.sending}>
